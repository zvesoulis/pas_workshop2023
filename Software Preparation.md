...
GNU Octave is a scientific programming lanaguage that is largely compatible with the commerical MATLAB platform, yet is available for free.  It is capable of loading large datasets, generating visualizations, and running analytic code.  GNU Octave can be obtained from: (https://octave.org/download).  For Windows users, scroll to the bottom and download the Windows 64-bit installer labelled **octave-8.1.0-w64-installer.exe.**
